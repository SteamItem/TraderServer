...
test('Low balance', () => {
  const filterer = new RollbitInventoryFilterer(0, [inventoryItem], [wishlistItem], loggerMock);
  filterer.filter();
  expect(filterer.itemsToBuy).toHaveLength(0);
});

test('Has balance with item', () => {
  const filterer = new RollbitInventoryFilterer(10, [inventoryItem], [wishlistItem], loggerMock);
  filterer.filter();
  expect(filterer.itemsToBuy).toHaveLength(1);
});

test('Ignore item with markup', () => {
  const filterer = new RollbitInventoryFilterer(10, [inventoryItemMarkup], [wishlistItem], loggerMock);
  filterer.filter();
  expect(filterer.itemsToBuy).toHaveLength(0);
});
